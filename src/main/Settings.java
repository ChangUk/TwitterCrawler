...
public class Settings {
	...
	public static boolean isValidUser(TwitterUser user) {
		if (user.isProtected())
			return false;
		if (user.isVerifiedCelebrity())					// If the user is a verified celebrity, the user is not normal user.
			return false;
		if (user.getFollowingsCount() > 5000)			// Requirement #1: maximum number of following users
			return false;
		if (user.getFollowersCount() > 5000)			// Requirement #2: maximum number of followers
			return false;
		if (user.getTweetsCount() < 1)					// Requirement #3: minimum number of tweets
			return false;
		return true;
	}
